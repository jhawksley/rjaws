...
impl AWSHandler {
    ...
    pub async fn instance_can_ssm(&mut self,
                                  instance: &Instance) -> bool
    {
        ...
        if self.instance_profile_cache.len() == 0 {
            ...
            for ip in client.list_instance_profiles().send()
                ...
                self.instance_profile_cache.insert(ip.arn().unwrap().to_string(), ip.clone());
            }
        }
        ...
        let ip = self.instance_profile_cache.get(instance_profile_arn).unwrap();
        ...
        for policy in policies.attached_policies().unwrap() {
            if policy.policy_name().unwrap() == "AmazonSSMManagedInstanceCore" {
                self.instance_profile_ssm_mapping_cache.insert(ip_arn.to_string(), true);
                return true;
            }
        }

        self.instance_profile_ssm_mapping_cache.insert(ip_arn.to_string(), false);
        false
    }
